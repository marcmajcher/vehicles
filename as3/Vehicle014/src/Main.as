package 
{
	...
	public class Main extends Sprite 
	{
		...
		private function init(e:Event = null):void 
		{
			...
			// add random targets and enemies
			for each (var v:Vehicle in _world.vehicles) {
				v.target = _world.vehicles[Math.floor(Math.random() * _world.vehicles.length)];
				v.enemy = _world.vehicles[Math.floor(Math.random() * _world.vehicles.length)];
			}
